...
func (m *CreateRequest) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetEmail()); l < 4 || l > 255 {
		err := CreateRequestValidationError{
			field:  "Email",
	...
	if l := utf8.RuneCountInString(m.GetPassword()); l < 7 || l > 50 {
		err := CreateRequestValidationError{
			field:  "Password",
			reason: "value length must be between 7 and 50 runes, inclusive",
		}
		if !all {
			return err
		}
	...
	if l := utf8.RuneCountInString(m.GetPasswordConfirm()); l < 7 || l > 50 {
		err := CreateRequestValidationError{
